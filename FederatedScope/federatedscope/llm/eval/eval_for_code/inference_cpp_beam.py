...
def cal(code, filename):
    ...
    output = pipe(prompt, min_length=cnt+64, max_length=cnt+max_d, temperature=1.0, do_sample=False, num_beams=10, num_beam_groups=10, diversity_penalty=1.0, num_return_sequences=10)
    full_text = [o['generated_text'] for o in output]
    # print(full_text)
    ret = [extract_first_cpp_code(ft.split(EOF)[1]) for ft in full_text]
...
for file_path in sorted(Path(base_dir).rglob('*.cpp'), reverse=True):
    ...
    for e, text in enumerate(zip(full, res)):
        ...
        with open(fix_name, 'w', encoding='utf-8') as file:
            print(text[1], file=file) # fixed code
