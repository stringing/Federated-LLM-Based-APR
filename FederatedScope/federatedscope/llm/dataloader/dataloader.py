    ...
    def __call__(self, instances):
        ...
        if instances[0]['mo']:
            ...
            for j, input_id in enumerate(input_ids):
                split_indices = [i for i, x in enumerate(input_id) if x == eos_token_id]
                pred_input_ids.append(input_id[:split_indices[0]+1])
                pred_labels.append(labels[j][:split_indices[0]+1])
                expl_input_ids.append(input_id[split_indices[0]+1:])
                expl_labels.append(labels[j][split_indices[0]+1:])
            
            pred_input_ids = torch.nn.utils.rnn.pad_sequence(
                pred_input_ids,
                batch_first=True,
                padding_value=self.tokenizer.pad_token_id)
            pred_labels = torch.nn.utils.rnn.pad_sequence(
                pred_labels,
                batch_first=True,
                padding_value=DefaultToken.IGNORE_INDEX.value)
            pred_attention_mask=pred_input_ids.ne(self.tokenizer.pad_token_id)
            
            expl_input_ids = torch.nn.utils.rnn.pad_sequence(
                expl_input_ids,
                batch_first=True,
                padding_value=self.tokenizer.pad_token_id)
            expl_labels = torch.nn.utils.rnn.pad_sequence(
                expl_labels,
                batch_first=True,
                padding_value=DefaultToken.IGNORE_INDEX.value)
            expl_attention_mask=expl_input_ids.ne(self.tokenizer.pad_token_id)
...
def get_tokenizer(model_name, cache_dir, tok_len=128, pkg='huggingface_llm'):
    ...
    tokenizer.pad_token = tokenizer.eos_token # try if fix the "using pad token, but it is not set" warning
...
def load_json(file_path,
    ...
    if mo:
        for item in list_data_dict:
            new_item = dict(
                input1=item['input1'] if 'input1' in item else None,
                input2=item['input2'] if 'input2' in item else None,
                output1=item['output1'] if 'output1' in item else None,
                output2=item['output2'] if 'output2' in item else None,
...
def load_llm_dataset(config=None, **kwargs):
    ...
    if dataset_name.endswith('.json'):
        ...
        list_data_dict = load_json(fp, config.data.mo)
        if config.data.sample_ratio != -1.0:
            print(f'--------------------sample ratio: {config.data.sample_ratio}---------------------------')
            sample_size = int(config.data.sample_ratio * len(list_data_dict))
            list_data_dict = random.sample(list_data_dict, sample_size)
            print(f'--------------------sampled: {len(list_data_dict)}---------------------------')
        dataset = LLMDataset(list_data_dict, tokenizer, config.data.mo)
