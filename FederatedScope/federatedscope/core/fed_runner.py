...
class StandaloneRunner(BaseRunner):
    def _set_up(self):
        ...
        self.fixed_clients = None # for efficient sampling
    ...
    def _remove_common_receivers(self, list1, list2):
        set1 = set(list1)
        set2 = set(list2)
    ...
    def _exchange_client_data(self, fixed_clients, new_clients):
        print(f'fixed clients are {fixed_clients}, swap with {new_clients}')
        fcs, ncs = self._remove_common_receivers(fixed_clients, new_clients)
        for i, j in zip(fcs, ncs):
            print(f'swap client data {i} with {j}')
            self.client[i].data, self.client[j].data = self.client[j].data, self.client[i].data
            self.client[i].trainer.ctx.data, self.client[j].trainer.ctx.data = self.client[j].trainer.ctx.data, self.client[i].trainer.ctx.data
            self.client[i].trainer.update_changed_data()
            self.client[j].trainer.update_changed_data()
    ...
    def _handle_msg(self, msg, rcv=-1):
        ...
        print(f'----------{receiver}-----------{msg.msg_type}')
        if self.cfg.federate.eff_samp and msg.msg_type == 'model_para' and receiver != [0]:
            print('-----------perform efficient sampling-------------')
            if self.fixed_clients is None:
                self.fixed_clients = receiver
            else:
                self._exchange_client_data(self.fixed_clients, receiver)
                receiver = self.fixed_clients
                
        # if self.cfg.federate.eff_samp and msg.msg_type == 'evaluate':
        #     return
